...
function confirm_delete_app(id, name) {
    ...
    bootbox.confirm({
        ...
        callback: function(confirmacao) {
            ...
            if (confirmacao)
                $.ajax({
                    url: window.location.origin+"/apps/delete/" + id,
                    type: "GET",
...
function create_app() {
    ...
    var data = {
        ...
        urlpost: $('input[name=urlpost]').val(),
        urlhome: $('input[name=urlhome]').val(),
        loading: $('input[name=loadingpost]:checked').val(),
    ...
    if (data.special == 3) {
        if (data.urlpost == "") {
            Toast.fire({
                icon: 'error',
                title: '&nbsp&nbsp Os campos URL são obrigatórios. '
            });
    ...
    if (data.urlhome == "") {
        Toast.fire({
            icon: 'error',
            title: '&nbsp&nbsp O campo URL da página inicial é obrigatório. '
        });
    ...
    $.ajax({
        ...
        url: window.location.origin+'/apps_add',
        type: 'POST',
        data: data,
        dataType: 'text',
        ...
        error: function(data) {
            Toast.fire({
                icon: 'error',
                title: '&nbsp&nbsp Erro ao criar aplicação.'
            });
        }
    });
}
...
function edit_app() {
    ...
    var data = {
        ...
        urlpost: $('input[name=url_post]').val(),
        urlhome: $('input[name=url_home]').val(),
        loading: $('input[name=loading_post]:checked').val(),
    ...
    $.ajax({
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        url: window.location.origin+'/app_edit',
        type: 'POST',
        data: data,
        dataType: 'text',
...
function link_login() {
    ...
    $.ajax({
        ...
        url: window.location.origin+'/link_login',
        type: 'POST',
        data: data,
        dataType: 'text',
